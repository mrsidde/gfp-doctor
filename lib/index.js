...
function autoFix(packages) {
    //packages = { install, uninstall}
    var installs = packages
        .filter(({ install }) => install)
        .map(({ install }) => install.join(' '))
        .reduce(
            (allPackages, currentPackage) => `${allPackages} ${currentPackage}`
        );
...
function printSummary(tests) {
    ...
    log.info('');
    log.info(
        `${emoji.checkered_flag}  Ran ${
            tests.length
        } tests, passed: ${passedCount}, failed: ${failedCount}, skipped: ${skippedCount}`
    );
